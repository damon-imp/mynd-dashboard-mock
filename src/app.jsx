...
function App() {
  ...
  const P = {
    ...
    revenue:<Revenue/>, products:<Products/>, subs:<Subs/>, wholesale:<Wholesale/>,
    attribution:<Attribution/>, ads:<Ads/>, social:<Social/>,
    inventory:<Inventory/>, production:<Production/>, suppliers:<Suppliers/>,
  ...
  return (
    <div className="shell" data-open={sideOpen}>
          ...
          {NAV.map(g=>{
            ...
            return (
              <div key={g.g} style={{ marginBottom:2 }}>
                ...
                {open && g.items.map(i=>(
                  <button key={i.id} className="nav-item" data-on={page===i.id} onClick={()=>setPage(i.id)}>{i.l}</button>
                ))}
              </div>
            );
          })}
      ...
      <div style={{ minWidth:0 }}>
        ...
        <main className="pad" style={{ padding:"26px 26px 70px", maxWidth:1680, margin:"0 auto" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
            ...
            <Seg options={["7 days","30 days","90 days","MTD","Custom"]} value={period} onChange={setPeriod}/>
          </div>
          {sub === 0 ? P : (
            <Empty title={`${subs[sub]} view`}

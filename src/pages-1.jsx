  ...
  return (
    <div className="page-in">
      ...
      <SecLabel icon="funnel" help="Where people fall out between landing on the site and rebilling a third time."
        right="30 day window · site to third rebill">The funnel · {period}</SecLabel>
  ...
  return (
    <div className="page-in">
      ...
      <G c={4} style={{ marginBottom:24 }}>
        ...
        <KPI label="Biggest dollar gap" value="$38K / mo" tone="warn" sub="revenue against target" />
      </G>
      <Card pad={22}>
        <SecLabel icon="target" help="Current against target, with the benchmark marked.">Scorecard</SecLabel>
        {D.goals.map(g=><GoalRow key={g.g} label={g.g} now={g.now} target={g.target}
          pct={g.pct} tone={g.tone} note={g.note} bench={g.bench} />)}
      </Card>
    </div>
  );
}
  ...
  return (
    <div className="page-in">
      ...
      <div style={{ textAlign:"center", marginBottom:26 }}>
        ...
        <Card pad={18} style={{ maxWidth:280, margin:"0 auto", borderColor:"var(--accent)" }}>
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:7 }}>
            <Avatar name="Damon B" size={40} />
            <div style={{ fontSize:15, fontWeight:600 }}>Damon B.</div>
            <div style={{ fontSize:11.5, color:"var(--accent)" }}>Founder / CEO</div>
            <Badge tone="bad">Every decision routes here</Badge>
          </div>
        </Card>
      </div>
      <SecLabel icon="team" right="4 teams">Teams and reporting</SecLabel>
      <G c={2} name="2" gap={14} style={{ marginBottom:20 }}>
        {D.org.teams.map(t=>(
          <Card key={t.team} pad={18} hover style={{ borderLeft:"3px solid var(--accent)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:9 }}>
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                <Avatar name={t.lead} size={32} />
                <div>
                  <div style={{ fontSize:14, fontWeight:600 }}>{t.lead}</div>
                  <div style={{ fontSize:11, color:"var(--accent)" }}>{t.team}</div>
                </div>
              </div>
              <Badge tone="mute">{t.people.length + 1} people</Badge>
            </div>
            <p style={{ fontSize:11.5, color:"var(--ink-soft)", marginBottom:12 }}>{t.note}</p>
            <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
              {t.people.map(p=>(
                <span key={p} style={{ display:"inline-flex", alignItems:"center", gap:6,
                  background:"var(--surface-3)", border:"1px solid var(--rule)", borderRadius:99,
                  padding:"4px 10px 4px 4px", fontSize:11.5 }}>
                  <Avatar name={p} size={19} tone="mute" />{p}
                </span>
              ))}
            </div>
          </Card>
      ...
      <Note tone="warn" icon="!">
        Nothing about how this business runs is written down and no role carries a number yet. That is the single
        biggest structural risk here, bigger than any individual metric on this dashboard.
      </Note>
    </div>
  );
}
...
/* ============================== SCORECARDS ============================== */
function Scorecards() {
  const tone = { good:"good", bad:"bad", mute:"mute" };
  const lab = { good:"On track", bad:"At risk", mute:"Not set" };
  return (
    <div className="page-in">
      <PageHead title="Role scorecards" sub="One number per seat, visible to the person being measured."
        meta="A scorecard nobody can see is a report about them, not a tool for them." />
      <Card pad={0}>
        <div className="scroll-x"><table className="tbl">
          <thead><tr><th>Who</th><th>Role</th><th>The one number</th>
            <th style={{textAlign:"right"}}>Now</th><th style={{textAlign:"right"}}>Target</th><th>Status</th></tr></thead>
          <tbody>{D.scorecards.map(s=>(
            <tr key={s.n}>
              <td><div style={{ display:"flex", alignItems:"center", gap:9 }}>
                <Avatar name={s.n} size={26} tone={s.st==="bad"?"bad":"accent"} />
                <span style={{ fontWeight:600 }}>{s.n}</span></div></td>
              <td style={{ color:"var(--ink-soft)" }}>{s.r}</td>
              <td>{s.metric}</td>
              <td className="num" style={{ textAlign:"right", fontWeight:600 }}>{s.now}</td>
              <td className="num" style={{ textAlign:"right", color:"var(--ink-soft)" }}>{s.target}</td>
              <td><Badge tone={tone[s.st]}>{lab[s.st]}</Badge></td>
            </tr>))}</tbody>
        </table></div>
      </Card>
    </div>
  ...
  return (
    <div className="page-in">
      ...
      <Note tone="bad" icon="!">
        ...
        $7,000 a month and it is the largest single lever left on the cost side.
      </Note>
      <div style={{ height:24 }} />
      <G c={2} name="2" gap={16}>
        <Card pad={20}>
...
function Rails() {
  ...
  return (
    <div className="page-in">
      ...
      <G c={4} style={{ marginBottom:24 }}>
        <KPI label="Gross" value={fmt.usd(tg)} tone="ink" sub="30 days, all rails" />
          ...
          <tbody>{D.rails.map(r=>(
            <tr key={r.n}>
              ...
              <td style={{ width:150 }}>{r.cap?<div>
                <Bar pct={(r.gross/r.cap)*100} tone={r.gross/r.cap>0.8?"bad":r.gross/r.cap>0.6?"warn":"good"} />
                <span className="mono" style={{ fontSize:9.5, color:"var(--ink-mute)" }}>{fmt.k(r.gross)} of {fmt.k(r.cap)}</span>

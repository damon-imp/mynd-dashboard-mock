  ...
  return (
    <div className="page-in">
      ...
      <G c={4} style={{ marginBottom:24 }}>
        <KPI label={D.unit[0].label} value={D.unit[0].value} tone="ink" delta={-4.1} spark={D.revMonthly.map(r=>r.v)} />
        <KPI label="Orders" value={fmt.n(D.funnel[3].v)} tone="ink" sub="on platform" delta={1.8} />
        <KPI label="Average order" value="$189" tone="ink" delta={2.1} help="Revenue over orders that touched the platform." />
        <KPI label="Unattributed" value="26.2%" tone="bad" sub={fmt.usd(D.channels[5].v) + " of revenue"} help="Orders without an affiliate land on an internal test account." />
      </G>
      ...
      <Note tone="info" icon="i">
        Shipments and fulfillment moved to Operations, where they belong. Retention now sits in its own page
        under Revenue.
      </Note>
    </div>
  );
    ...
    <div className="page-in">
      ...
      <Card pad={20} style={{ marginBottom:20 }}>
        ...
        <p style={{ fontSize:11.5, color:"var(--ink-mute)", marginTop:12 }}>
          Fell to 27.3% when subscriber payment credentials broke. Restored and recovering. The retry rebuild
          closes the rest.
        </p>
      </Card>
      <Note tone="info" icon="i">
...
function Ads() {
  return (
    <div className="page-in">
      ...
      <Note tone="warn" icon="!">
        Before spend turns back on you need a cost-per-customer ceiling that finance sets and marketing can't move.
        That number can't be computed until attribution is rebuilt, which makes the rebuild a prerequisite for the
